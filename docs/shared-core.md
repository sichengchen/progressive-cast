...
Swift uses `RajioCore.parseFeed(feedUrl:xml:fetchedAt:)`. The wrapper borrows UTF-8 input for the call and releases every Rust response through `rajio_core_free`. Link the Rust library built for the consuming Apple target. iOS artifact packaging and GRDB adapters are subsequent integration work.
