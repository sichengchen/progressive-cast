...
export class DownloadService {
  ...
  private static downloadCallbacks = new Map<string, (progress: DownloadProgress) => void>();
   ...
   */
  static async queueDownload(episode: Episode, priority: number = 1): Promise<void> {
    ...
    const alreadyQueued = existingQueue.some(item => item.episodeId === episode.id);
  ...
  static async processQueue(): Promise<void> {
    ...
    try {
      while (true) {
        ...
        // Get episode details
        const episode = await DatabaseService.getEpisodeById(nextItem.episodeId);
        if (!episode) {
        ...
        await DatabaseService.updateDownloadQueueStatus(nextItem.id, 'downloading');
   ...
   */
  private static async startDownload(episode: Episode, queueId: string): Promise<void> {
    ...
    try {
      ...
      // Remove from queue
      await DatabaseService.updateDownloadQueueStatus(queueId, 'completed');
   ...
   */
  static onProgress(episodeId: string, callback: (progress: DownloadProgress) => void): () => void {
   ...
   */
  private static notifyProgress(episodeId: string, progress: DownloadProgress): void {
   ...
   */
  static async getDownloadStatus(episodeId: string): Promise<DownloadProgress | null> {
    const progress = await DatabaseService.getDownloadProgress(episodeId);
