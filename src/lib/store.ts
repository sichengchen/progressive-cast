...
const ensureCompletePreferences = (
        ...
        (set, get) => ({
            ...
            initializeStore: async () => {
                try {
                    ...
                    const [
                        ...
                        DatabaseService.exportData().then(
                            (data) => data.playbackProgress
                        ),
            ...
            subscribeToPodcast: async (feedUrl: string) => {
                try {
                    ...
                    const existing = get().podcasts.find(
                        (p) => p.feedUrl === feedUrl
                    );
                    if (existing) {
                        closeProgressDialog();
                    ...
                    const episodes = RSSService.rssEpisodesToEpisodes(
                        feed.episodes,
                        podcast.id
                    );
            ...
            refreshPodcast: async (podcastId: string) => {
                try {
                    ...
                    const newEpisodes = RSSService.rssEpisodesToEpisodes(
                        feed.episodes,
                        podcast.id
                    );
                    ...
                    await DatabaseService.updatePodcast(podcast.id, {
                        lastUpdated: new Date(),
                    });
                ...
                try {
                    ...
                    await Promise.all(
                        podcasts.map((podcast) =>
                            get().refreshPodcast(podcast.id)
                        )
                    );
                } catch (error) {
                    set({
                        error: `Failed to refresh podcasts: ${
                            error instanceof Error
                                ? error.message
                                : "Unknown error"
                        }`,
            ...
            loadEpisodes: async (podcastId: string) => {
                try {
                    const episodes =
                        await DatabaseService.getEpisodesByPodcastId(podcastId);
                    set({ episodes });
                } catch (error) {
                    set({
            ...
            getLatestEpisodes: async () => {
                try {
                    ...
                    if (
                        ...
                        Date.now() - latestEpisodesCache.timestamp <
                            cacheValidDuration
                    ) {
                    ...
                    const latestEpisodes =
                        await DatabaseService.getLatestEpisodesOptimized(limit);
            ...
            getUnfinishedEpisodes: async () => {
                try {
                    ...
                    for (const episodeId of unfinishedProgressIds) {
                        try {
                            const episode =
                                await DatabaseService.getEpisodeById(episodeId);
                            if (episode) {
                    ...
                    const sortedEpisodes = episodes.sort(
                        (a: Episode, b: Episode) => {
                            ...
                            return (
                                new Date(progressB.lastPlayedAt).getTime() -
                                new Date(progressA.lastPlayedAt).getTime()
                            );
                        }
                    );
                    ...
                    set({
                        error: `Failed to load unfinished episodes: ${
                            error instanceof Error
                                ? error.message
                                : "Unknown error"
                        }`,
            ...
            markEpisodeCompleted: async (episodeId: string) => {
                ...
                await DatabaseService.markEpisodeCompleted(
                    episodeId,
                    playbackState.currentEpisode.podcastId
                );
            ...
            setProgressDialog: (data: {
                ...
                set((state) => ({
                    progressDialog: {
                        ...
                        currentItem:
                            data.currentItem ||
                            state.progressDialog.currentItem,
                        progress:
                            data.progress !== undefined
                                ? data.progress
                                : state.progressDialog.progress,
                        total:
                            data.total !== undefined
                                ? data.total
                                : state.progressDialog.total,
                    },
                }));
            },
            ...
            updateProgress: (progress: number, currentItem?: string) => {
                set((state) => ({
                    progressDialog: {
                        ...
                        progress,
                        currentItem:
                            currentItem || state.progressDialog.currentItem,
                    },
                }));
            },
                    ...
                    for (let i = 0; i < outlines.length; i++) {
                        ...
                        if (feedUrl) {
                            try {
                                ...
                                const feed = await RSSService.parseFeed(
                                    feedUrl
                                );
                                const podcast =
                                    RSSService.rssFeedToPodcast(feed);
                                const episodes =
                                    RSSService.rssEpisodesToEpisodes(
                                        feed.episodes,
                                        podcast.id
                                    );
                                ...
                                console.error(
                                    `Failed to import podcast: ${feedUrl}`,
                                    error
                                );
                                errors++;
                            }
            ...
            downloadEpisode: async (episode: Episode) => {
                try {
                    ...
                    DownloadService.setProgressCallback(
                        episode.id,
                        (progress: DownloadProgress) => {
            ...
            cancelDownload: async (episodeId: string) => {
                try {
                    ...
                    set((state) => {
                        const newDownloadProgress = new Map(
            ...
            deleteDownload: async (episodeId: string) => {
                try {
                    ...
                    const shouldClearPlayback =
                        playbackState.currentEpisode?.id === episodeId;
                    ...
                    set((state) => ({
                        ...
                        downloadedEpisodes: state.downloadedEpisodes.filter(
                            (ep) => ep.id !== episodeId
                        ),
                        downloadProgress: new Map(
                            [...state.downloadProgress].filter(
                                ([key]) => key !== episodeId
                            )
                        ),
            ...
            getDownloadedEpisodes: async () => {
                try {
                    const downloadedEpisodes =
                        await DatabaseService.getDownloadedEpisodes();
                    set({ downloadedEpisodes });
                    return downloadedEpisodes;
                } catch (error) {
                    set({
                        error: `Failed to get downloaded episodes: ${
                            error instanceof Error
                                ? error.message
                                : "Unknown error"
                        }`,
            ...
            updateDownloadProgress: (
                ...
                set((state) => {
                    ...
                    if (progress.status === "completed") {
                        ...
                        const completedEpisode = updatedEpisodes.find(
                            (ep) => ep.id === episodeId
                        );
                        const updatedDownloadedEpisodes =
                            completedEpisode &&
                            !state.downloadedEpisodes.find(
                                (ep) => ep.id === episodeId
                            )
                                ? [
        ...
        }),
        {
            ...
            merge: (persistedState, currentState) => {
                const persisted = persistedState as
                    | Partial<PodcastStore>
                    | undefined;
                return {
                    ...currentState,
                    ...(persisted || {}),
                    preferences: ensureCompletePreferences(
