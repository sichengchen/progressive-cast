...
export class RSSService {
    static async parseFeed(feedUrl: string): Promise<RSSFeed> {
        try {
            ...
            if (
                ...
                console.error(
                    "Invalid XML format. Content preview:",
                ...
                feed.items?.map((item: any) => {
                    return {
                        ...
                        showNotes:
                            ...
                            item.itunes.summary ||
                            item.summary ||
                            "", // Prioritize content:encoded for rich show notes
    ...
    private static extractAudioUrl(item: any): string {
        ...
        if (item.content) {
            const audioMatch = item.content.match(
                /src="([^"]*\.(mp3|m4a|ogg|wav))"/i
            );
            if (audioMatch) {
                return audioMatch[1];
            }
    ...
    private static extractCategories(feed: any): string[] {
        ...
        if (feed.itunes.category) {
            if (Array.isArray(feed.itunes.category)) {
                ...
                categories.push(
                    feed.itunes.category.text || feed.itunes.category
                );
            }
        }
    ...
    private static generateEpisodeId(
        podcastId: string,
        title: string,
        publishedAt: Date
    ): string {
