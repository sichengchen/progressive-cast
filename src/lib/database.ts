...
export interface PodcastDB {
    ...
    audioFiles: EntityTable<
        { key: string; blob: Blob; size: number; createdAt: Date },
        "key"
    >;
}
...
db.version(2).stores({
    podcasts: "&id, title, feedUrl, subscriptionDate, lastUpdated",
    episodes:
        "&id, podcastId, title, publishedAt, audioUrl, [publishedAt+podcastId]", // Compound index for latest episodes query
    playbackProgress: "&id, episodeId, podcastId, lastPlayedAt, isCompleted",
});
...
db.version(3).stores({
    podcasts: "&id, title, feedUrl, subscriptionDate, lastUpdated",
    episodes:
        "&id, podcastId, title, publishedAt, audioUrl, [publishedAt+podcastId], isDownloaded", // Added isDownloaded index
    playbackProgress: "&id, episodeId, podcastId, lastPlayedAt, isCompleted",
...
export class DatabaseService {
    ...
    static async getLatestEpisodesOptimized(
        limit: number = 10
    ): Promise<Episode[]> {
    ...
    static async markEpisodeCompleted(
        episodeId: string,
        podcastId: string
    ): Promise<void> {
    ...
    static async getDownloadedEpisodes(): Promise<Episode[]> {
        return await db.episodes
            .filter((episode) => episode.isDownloaded === true)
            .toArray();
    }
    ...
    static async getStorageStats(): Promise<{
        ...
        const totalSize = downloadedEpisodes.reduce(
    ...
    static async saveDownloadProgress(
