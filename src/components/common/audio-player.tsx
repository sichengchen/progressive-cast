    ...
    const updateMediaSession = useCallback(() => {
        if (
            ...
            navigator.mediaSession.setActionHandler(
                ...
                (details) => {
                    const skipTime =
                        details.seekOffset || preferences.skipInterval || 30;
                    handleSkip(-skipTime);
                }
            );
            ...
            navigator.mediaSession.setActionHandler("seekto", (details) => {
                if (details.seekTime !== undefined && audioRef.current) {
                    const newTime = Math.max(
                        0,
                        Math.min(duration, details.seekTime)
                    );
                    setCurrentTime(newTime);
                    audioRef.current.currentTime = newTime;
    ...
    useEffect(() => {
        ...
        const loadAudioSource = async () => {
            ...
            if (currentEpisode.isDownloaded) {
                try {
                    const localUrl = await DownloadService.getLocalAudioUrl(
                        currentEpisode
                    );
                    if (localUrl) {
                        audioUrl = localUrl;
                        console.log(
                            "Using local audio file for offline playback"
                        );
                    } else {
                        console.log(
                            "Local file missing, falling back to streaming"
                        );
                    }
                } catch (error) {
                    console.warn(
                        "Failed to load local audio file, falling back to streaming:",
                        error
                    );
                }
            }
        ...
        const handleLoadedMetadata = () => {
            ...
            const savedProgress = currentEpisode
                ? playbackProgress.get(currentEpisode.id)
                : null;
        ...
        const handleError = (e: Event) => {
            ...
            if (audio.src.startsWith("blob:") && currentEpisode.isDownloaded) {
                console.log(
                    "Local playback failed, attempting to fallback to streaming"
                );
                URL.revokeObjectURL(audio.src);
