...
export function ShowNotes() {
  ...
  const content = currentEpisode?.showNotes || currentEpisode?.content || currentEpisode?.description || '';
  ...
  const processContent = (html: string) => {
    ...
    if (!sanitized.includes('<p>') && !sanitized.includes('<div>') && !sanitized.includes('<br>') && !sanitized.includes('<ol>') && !sanitized.includes('<ul>')) {
    ...
    sanitized = sanitized.replace(
      ...
      (match, url, offset, fullString) => {
        ...
        if (afterMatch.indexOf('</a>') < afterMatch.indexOf('<a') && afterMatch.indexOf('</a>') !== -1) {
          return match;
        }
    ...
    sanitized = sanitized.replace(
      ...
      (match, email) => {
        ...
        return match.includes('href=') || match.includes('mailto:') ? match : `<a href="mailto:${email}">${email}</a>`;
      }
    );
