...
import { Download, CheckCircle, AlertCircle, X, RotateCcw, Trash2 } from 'lucide-react';
  ...
  const getButtonContent = () => {
    ...
    if (pageType === 'downloaded') {
      if (episode.isDownloaded) {
        ...
        return {
          icon: showTrashIcon ? <Trash2 className="w-4 h-4 text-destructive" /> : <CheckCircle className="w-4 h-4" />,
          tooltip: isMobile ? 'Downloaded - Click to remove' : (isHovered ? 'Downloaded - Click to remove' : 'Downloaded'),
          action: handleDeleteClick,
          variant: 'ghost' as const,
          className: showTrashIcon ? 'text-destructive' : 'text-green-600'
        };
      }
    ...
    <div className="flex flex-col items-center gap-1">
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent onClick={(e) => e.stopPropagation()}>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Downloaded Episode</AlertDialogTitle>
