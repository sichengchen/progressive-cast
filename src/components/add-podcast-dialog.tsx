...
export function AddPodcastDialog({ open, onOpenChange }: AddPodcastDialogProps) {
  ...
  const { subscribeToPodcast, clearError, progressDialog, podcasts, preferences } = usePodcastStore();
  ...
  const handleItunesSearch = async (e: React.FormEvent) => {
    ...
    setHasSearched(true);
    try {
      const results = await iTunesService.searchPodcasts(searchQuery.trim(), 20);

      // Filter out already subscribed podcasts
      const unsubscribedPodcasts = results.results.filter(itunesPodcast =>
        !podcasts.some(localPodcast => localPodcast.feedUrl === itunesPodcast.feedUrl)
      );

      setItunesResults(unsubscribedPodcasts);

      if (unsubscribedPodcasts.length === 0) {
        toast.info('No new podcasts found or all results are already subscribed');
      }
    } catch (error) {
  ...
  const handleSubscribeFromItunes = async (itunesPodcast: iTunesPodcast) => {
    ...
    try {
      ...
      setItunesResults(prev => prev.filter(p => p.id !== itunesPodcast.id));
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Add a podcast by RSS feed URL or search iTunes for new podcasts.
          </DialogDescription>
        </DialogHeader>

        <div className="flex-1 min-h-0 overflow-hidden">
          <Tabs defaultValue={preferences.itunesSearchEnabled ?? true ? "search" : "url"} className="h-full flex flex-col">
            {(preferences.itunesSearchEnabled ?? true) && (
              <TabsList className="grid w-full grid-cols-2">
                <TabsTrigger value="search" disabled={progressDialog.isOpen}>
                  Search iTunes
                </TabsTrigger>
                <TabsTrigger value="url" disabled={progressDialog.isOpen}>
                  RSS URL
                </TabsTrigger>
              </TabsList>
            )}

            <TabsContent value="url" className="flex-1 min-h-0 overflow-hidden">
            ...
            {(preferences.itunesSearchEnabled ?? true) && (
              <TabsContent value="search" className="flex-1 min-h-0 overflow-hidden data-[state=active]:flex data-[state=active]:flex-col">
                <div className="flex-1 min-h-0 pt-4 pb-2 flex flex-col">
                  <form onSubmit={handleItunesSearch} className="space-y-4 flex-shrink-0">
                    <div>
                      ...
                      <div className="flex gap-2 mt-1">
                        <Input
                          ...
                          disabled={isSearching || progressDialog.isOpen}
                          className="flex-1 focus-visible:ring-0 focus-visible:ring-offset-0"
                        />
                        <Button
                          type="submit"
                          disabled={isSearching || !searchQuery.trim() || progressDialog.isOpen}
                          className="px-4"
                        >
                  ...
                  <div className="flex-1 min-h-0 mt-4 flex flex-col">
                    {isSearching ? (
                      <div className="flex items-center justify-center py-8">
                        <div className="text-center text-muted-foreground">
                          <div className="mb-2">Searching iTunes...</div>
                              ...
                              <CardContent className="p-4">
                                <div className="flex items-start gap-3">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    ...
                                    {podcast.genre && (
                                      <p className="text-xs text-muted-foreground">{podcast.genre}</p>
                                    )}
                                    {podcast.description && (
                                  ...
                                  <div className="flex items-center gap-2 flex-shrink-0">
                                    {podcast.itunesUrl && (
                                      <Button
                                        size="sm"
                                        variant="ghost"
                                        onClick={() => window.open(podcast.itunesUrl, '_blank')}
                                    ...
                                    <Button
                                      size="sm"
                                      onClick={() => handleSubscribeFromItunes(podcast)}
                                      disabled={!podcast.feedUrl || progressDialog.isOpen}
                                      className="h-8 px-3"
                                    >
                    ...
                    ) : hasSearched && itunesResults.length === 0 && !isSearching ? (
                      <div className="flex items-center justify-center py-8">
                        <div className="text-center text-muted-foreground">
                          <div className="mb-2">No podcasts found</div>
