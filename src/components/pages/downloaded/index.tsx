...
export function DownloadedPage() {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                setActiveDownloads(
                    activeProgress.filter((p) => p.status === "downloading")
                );
    ...
    useEffect(() => {
        const activeProgressArray = Array.from(
    ...
    const handleCancelDownload = async (episodeId: string) => {
        try {
            ...
            setActiveDownloads((prev) =>
                prev.filter((p) => p.episodeId !== episodeId)
            );
        } catch (error) {
                    ...
                    {downloadedEpisodes.length === 0 ? (
                        <div className="text-center py-8 text-muted-foreground">
                            ...
                            <p className="text-sm mt-1">
                                Download episodes from the podcast view to play
                                them offline
                            </p>
                        </div>
                    ) : (
                        <div className="space-y-2">
                            {downloadedEpisodes.map((episode, index) => (
                                <div key={episode.id}>
                                    ...
                                    {index < downloadedEpisodes.length - 1 && (
                                        <Separator />
                                    )}
                                </div>
                            ))}
