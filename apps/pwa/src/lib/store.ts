    ...
    (set, get) => ({
      ...
      initializeStore: async () => {
        try {
          ...
          const [podcasts, allProgress, downloadProgress, downloadedEpisodes, storageStats] =
            await Promise.all([
      ...
      deleteDownload: async (episodeId: string) => {
        try {
          ...
          set((state) => ({
            ...
            downloadProgress: new Map(
              [...state.downloadProgress].filter(([key]) => key !== episodeId),
            ),
        ...
        set((state) => {
          ...
          if (progress.status === "completed") {
            const updatedEpisodes = state.episodes.map((ep) =>
              ep.id === episodeId ? { ...ep, isDownloaded: true } : ep,
            );
            ...
            const updatedDownloadedEpisodes =
              completedEpisode && !state.downloadedEpisodes.find((ep) => ep.id === episodeId)
