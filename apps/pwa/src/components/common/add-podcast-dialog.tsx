...
export function AddPodcastDialog({ open, onOpenChange }: AddPodcastDialogProps) {
  ...
  const { subscribeToPodcast, clearError, progressDialog, podcasts, preferences } =
    usePodcastStore();

  const searchMutation = useMutation({
    mutationFn: ({ limit, term }: { limit: number; term: string }) =>
      iTunesService.searchPodcasts(term, limit),
  });
  ...
  const handleSubscribeFromItunes = async (itunesPodcast: iTunesPodcast) => {
    ...
    try {
      await subscribeMutation.mutateAsync(itunesPodcast.feedUrl);
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 min-h-0 overflow-hidden">
          <Tabs
            defaultValue={(preferences.itunesSearchEnabled ?? true) ? "search" : "url"}
            className="h-full flex flex-col"
          >
            {(preferences.itunesSearchEnabled ?? true) && (
              <TabsList className="grid w-full grid-cols-2">
                <TabsTrigger value="search" disabled={isSubmitting}>
                  Search iTunes
                </TabsTrigger>
                    ...
                    {isSearching ? (
                                ...
                                <div className="flex items-start gap-3">
                                  ...
                                  <div className="flex items-center gap-2 flex-shrink-0">
                                    ...
                                    <Button
                                      ...
                                      disabled={!podcast.feedUrl || isSubmitting}
                                      className="h-8 px-3"
                                    >
                      ...
                      <div className="flex items-center justify-center py-8">
                        <div className="text-center text-muted-foreground">
                          ...
                          <div className="text-sm">
                            Enter keywords to discover podcasts you haven&apos;t subscribed to
                          </div>
                        </div>
                      </div>
                    )}
