...
import { Sparkles, Settings, Radio, ArrowLeft, Plus, RefreshCw } from "lucide-react";
    ...
    <>
      ...
      {isMobile && (
        <MobileTabBar
          ...
          activeTab={
            ["podcasts", "resume-playing", "downloaded"].includes(currentPage)
              ? "library"
              : currentPage
          }
          onTabChange={(tabId) => {
      ...
      <AddPodcastDialog open={showAddPodcastDialog} onOpenChange={setShowAddPodcastDialog} />
    </>
  );
};
...
export const MainContent = () => {
  ...
  const refreshButton: ToolBarItemButton = {
    inner: <RefreshCw className={`h-3 w-3 ${isRefreshing ? "animate-spin" : ""}`} />,
    action: async () => {
      try {
  ...
  if (currentPage === "library") {
    return (
      <MainContentLayout title="Library" toolBar={[refreshButton, addButton]}>
        <LibraryPage />
      </MainContentLayout>
    );
  }
