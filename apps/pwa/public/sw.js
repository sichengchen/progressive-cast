...
const VERSION = isDev ? `dev-${Date.now()}` : "0.9.0-c22d3db";
  ...
  event.waitUntil(
    Promise.all([
      ...
      fetch("/offline.html")
        .then((response) => {
          if (response.ok) {
            return caches.open(STATIC_CACHE).then((cache) => cache.put("/offline.html", response));
          }
        })
...
self.addEventListener("message", (event) => {
  ...
  switch (type) {
    ...
    case "CLEAR_CACHE":
      ...
      break;
    case "CACHE_AUDIO":
      void cacheAudioFile(payload.url).then((success) => {
